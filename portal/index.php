...
function DisplayMainMenu(WebPage $oP)
{
	...
	$oP->AddMenuButton('showworkorder', 'Portal:ShowWorkOrder', '../portal/index.php?operation=show_work_order'); // ADDED BY Thiago Arcanjo
 ...
 * @return void
 */
function ShowWorkOrder(WebPage $oP)
{
        $oP->add("<div id=\"#work_order\">\n");
        $oP->add("<h1 id=\"#title_work_order\">".Dict::S('Portal:ListWorkOrder')."</h1>\n");
        ListWorkOrder($oP);
        $oP->add("</div>\n");
}
...
/**
 * CREATED BY Thiago Arcanjo
 * Lists all the work order for the current user
 * @param WebPage $oP The current web page
 * @return void
 */
function ListWorkOrder(WebPage $oP)
{
        $oUserOrg = GetUserOrg();
	...
	$sOQL = constant($sValidationDefine);
        ...
        $aClassToSet[$sClass] = new CMDBObjectSet($oSearch, array(), array());
	   ...
	   $oP->SetWelcomeMessage(Dict::Format('Portal:WelcomeUserOrg', UserRights::GetUserFriendlyName(), $oUserOrg->GetName()));
		...
		if (is_object($oUserOrg))
		{
			switch($sOperation)
			{
				...
				// ADDED BY Thiago Arcanjo
                                case 'show_work_order':
                                $oP->set_title(Dict::S('Portal:ShowWorkOrder'));
				DisplayMainMenu($oP);
                                ShowWorkOrder($oP);
                                break;
